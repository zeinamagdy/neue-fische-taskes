/*
Write a function that will return the count of distinct case-insensitive alphabetic characters and numeric digits that occur more 
than once in the input string. 
The input string can be assumed to contain only alphabets (both uppercase and lowercase) and numeric digits.
Example
"abcde" -> 0 # no characters repeats more than once
"aabbcde" -> 2 # 'a' and 'b'
"aabBcde" -> 2 # 'a' occurs twice and 'b' twice (`b` and `B`)
"indivisibility" -> 1 # 'i' occurs six times
"Indivisibilities" -> 2 # 'i' occurs seven times and 's' occurs twice
"aA11" -> 2 # 'a' and '1'
"ABBA" -> 2 # 'A' and 'B' each occur twice
*/

import { test } from "@/test.ts";

function duplicateCount(text: string): number {
  // your code here
  const duplicatedChars = new Map<string, number>();

  for (const char of text) {
    duplicatedChars.set(char.toLowerCase(), (duplicatedChars.get(char.toLowerCase()) ?? 0) + 1);
  }
  let count = 0;
  for (const [key, value] of duplicatedChars.entries()) {
    if (value > 1) {
      count++;
    }
  }
  return count;
}
console.log("duplicatedcount", duplicateCount("abcde"));

test(duplicateCount("abcde"), 0);
test(duplicateCount("aabbcde"), 2);
test(duplicateCount("aabBcde"), 2);
test(duplicateCount("indivisibility"), 1);
test(duplicateCount("ABBA"), 2);
