...
function infected(s: string): number {
  ...
  let infectedCount: number = s.split("X").reduce((acc: number, val: string) => {
    if (val.includes("1")) return acc + val.length;
    return acc;
  }, 0);
  return (100 * infectedCount) / s.split("").filter((el) => el !== "X").length;
}
