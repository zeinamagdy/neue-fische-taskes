...
function getPINs(s: string): string[] {
    ...
    s.split('').forEach((c: string) => numbers.push(neighbours[c]));
    return numbers.reduce((a: string[], b: string[]) => a.reduce((r: string[], v: string) => r.concat(b.map((w: string) => [].concat(v as any, w as any).join(''))), []), [''] as string[]);
}
